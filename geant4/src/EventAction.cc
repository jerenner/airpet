...
EventAction::EventAction()
  ...
  fTrackOutputDirCmd->SetGuidance(
      "Set the output directory for trajectory files.");
  fTrackOutputDirCmd->SetParameter(
      new G4UIparameter("dir", 's', false)); // 's' for string
  fTrackOutputDirCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  // Command for track visualization event range
  fSetTrackEventRangeCmd =
      new G4UIcommand("/g4pet/event/setTrackEventRange", this);
  fSetTrackEventRangeCmd->SetGuidance(
      "Set the range of event IDs for which to save tracks.");
  fSetTrackEventRangeCmd->SetGuidance(
      "Usage: /g4pet/event/setTrackEventRange startEventID endEventID");
...
void EventAction::SetTrackEventRange(G4int start, G4int end) {
  ...
  G4cout << "Track saving range set to events " << start << " through " << end
         << G4endl;
}
...
void EventAction::EndOfEventAction(const G4Event *event) {
  ...
  if (!runAction) {
    G4Exception("EventAction::EndOfEventAction()", "Event002", FatalException,
                "RunAction not found.");
    return;
  }
  ...
  if (runAction->GetSaveHits()) {
    ...
    if (!hce) {
      G4Exception("EventAction::EndOfEventAction()", "Event001", JustWarning,
    ...
    for (G4int cID : fHitsCollectionIDs) {
      auto hitsCollection =
          static_cast<AirPetHitsCollection *>(hce->GetHC(cID));
      if (hitsCollection) {
        for (size_t i = 0; i < hitsCollection->GetSize(); ++i) {
          ...
          if (hit->GetEdep() < runAction->GetHitEnergyThreshold())
            continue;
    ...
    if (trajectoryContainer) {
      for (size_t i = 0; i < trajectoryContainer->size(); ++i) {
        ...
        if (traj) {
          ...
          analysisManager->FillNtupleDColumn(0, 7,
                                             traj->GetInitialPosition().z());
          analysisManager->FillNtupleDColumn(0, 8, traj->GetInitialTime());
          analysisManager->FillNtupleDColumn(0, 9,
                                             traj->GetFinalPosition().x());
          analysisManager->FillNtupleDColumn(0, 10,
                                             traj->GetFinalPosition().y());
          analysisManager->FillNtupleDColumn(0, 11,
                                             traj->GetFinalPosition().z());
          analysisManager->FillNtupleDColumn(0, 12, traj->GetFinalTime());
          analysisManager->FillNtupleDColumn(0, 13,
                                             traj->GetInitialMomentum().x());
          analysisManager->FillNtupleDColumn(0, 14,
                                             traj->GetInitialMomentum().y());
          analysisManager->FillNtupleDColumn(0, 15,
                                             traj->GetInitialMomentum().z());
          analysisManager->FillNtupleDColumn(0, 16,
                                             traj->GetFinalMomentum().x());
          analysisManager->FillNtupleDColumn(0, 17,
                                             traj->GetFinalMomentum().y());
          analysisManager->FillNtupleDColumn(0, 18,
                                             traj->GetFinalMomentum().z());
...
void EventAction::WriteTracksToFile(const G4Event *event) {
  ...
  filename << fTrackOutputDir << "/event_" << std::setw(4) << std::setfill('0')
           << event->GetEventID() << "_tracks.txt";

  std::ofstream outFile(filename.str());
  if (!outFile.is_open()) {
    G4cerr << "ERROR: Could not open track output file: " << filename.str()
           << G4endl;
    return;
  }
  ...
  for (size_t i = 0; i < trajectoryContainer->size(); ++i) {
    auto traj = dynamic_cast<AirPetTrajectory *>((*trajectoryContainer)[i]);
    if (traj) {
      outFile << "T " << event->GetEventID() << " " << traj->GetParticleName()
              << " " << traj->GetTrackID() << " " << traj->GetParentID() << " "
              << traj->GetPDGEncoding() << "\n";
