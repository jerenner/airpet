   ...
   fSaveHits(true)
{
  ...
  fSaveParticlesCmd->SetParameter(new G4UIparameter("value", 'b', true)); // 'b' for boolean
...
void RunAction::BeginOfRunAction(const G4Run* /*aRun*/)
{
  ...
  if(fSaveHits) {
    ...
    analysisManager->CreateNtupleSColumn("PhysicalVolumeName"); // 2 (Name of the LV)
    ...
    analysisManager->FinishNtuple(fSaveParticles); // Finalize the second n-tuple
  }
}
