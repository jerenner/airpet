...
def create_silicon_slab_v3_project(output_path: Path) -> dict:
    ...
    _must(
        *pm.upsert_param_study("silicon_slab_v3_1", {
            "name": "silicon_slab_v3_1",
            "mode": "random",
            ...
            "objectives": [
                ...
                {"metric": "formula", "name": "area_norm", "direction": "minimize", "expression": "(half_x_mm/12.5) * (half_y_mm/12.5)"},
                {"metric": "formula", "name": "thickness_norm", "direction": "minimize", "expression": "thickness_mm/3.0"},
                {"metric": "formula", "name": "cost_norm", "direction": "minimize", "expression": "area_norm * thickness_norm"},
                {"metric": "formula", "name": "distance_norm", "direction": "minimize", "expression": "abs(src_z_mm - slab_z_mm)/20.0"},
                {"metric": "formula", "name": "score", "direction": "maximize", "expression": "0.8*edep_sum - 0.15*cost_norm - 0.05*distance_norm"},
            ],
        }),
        context="upsert_param_study(v3_1)",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a 5-parameter Silicon Slab v3.1 simulation-in-loop benchmark project.")
    parser.add_argument(
        "--output",
        default="surrogate/benchmarks/silicon_slab_v3/project.json",
        help="Output project JSON path.",
    )
    args = parser.parse_args()

    summary = create_silicon_slab_v3_project(Path(args.output).expanduser().resolve())
